...
This module is a sip implementation in python 
...
class SipMachine:
    ...
    def __init__(self: str, user: str, pwd: str, telNr: str, userAgent: str, userClient: str, proxyServer: str, proxyRegistrar: str = '192.168.1.1', port: str = 5060):
        ...
        self.__sock_sip_r.bind(socket.getaddrinfo(
            self.__userA.userClient, port)[0][-1])
        self.__sock_sip_r.listen(0)

        self.__sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.__sock.bind(socket.getaddrinfo(
        ...
        for fd in ready_list:
            if fd[1] & select.POLLIN:
                ...
                if fd[0] == self.__sock_sip_r.fileno() or fd[0] == self.__sock_sip_r:
                    ...
                    a = self.__sock_sip_r.accept()
                    self.__setConnection(a[0])
        gc.collect()
        if self.__call:
            if debug:
                print('start')
    ...
    def bye(self, auth=None):
        ...
        self.__writeSIPdata(
            'BYE sip:{}@{} SIP/2.0{}'.format(self.__userA.telNr, self.__userA.userClient, self.__RN))
    ...
    def __sipRegister(self, userA: UserA, auth: Auth):
        self.__setConnection()
        userB = UserB()
        userB.telNr = None
        ...
        self.__writeSIPdata(
            'REGISTER sip:{} SIP/2.0{}'.format(self.__proxyRegistrar, self.__RN))
        self.__getVia(userA)
        self.__getMaxForwards()
        self.__getTo(userA)
    ...
    def __sipInvite(self, userB: UserB, userA: UserA, auth: Auth):
        self.__userB.tagTo = ''
        conten = self.__getConten(userB, userA.userClient)
        contentLength = len(conten)
        if auth.nonce is not None:
    ...
    def __getFrom(self, userA: UserA):
        self.__writeSIPdata(
            'From: <sip:{}@{}>;tag={}{}'.format(userA.telNr, userA.agent, userA.tagFrom, self.__RN))

    def __getCallID(self, userA: UserA):
        self.__writeSIPdata(
            'Call-ID: {}@{}{}'.format(userA.callId, userA.agent, self.__RN))

    def __getCSeq(self, cSeq, typ):
        self.__writeSIPdata('CSeq: {} {}{}'.format(cSeq, typ, self.__RN))

    def __getContact(self, user: User):
    ...
    def __getContentLength(self, contentLength=0):
        self.__writeSIPdata(
            'Content-Length: {}{}'.format(contentLength, self.__RN))

    def __getACK(self, telNr, agent, port):
        self.__writeSIPdata(
    ...
    def __getInvite(self, user: User):
        self.__writeSIPdata(
    ...
    def __exec(self):
        if self.CSeqTyp == self.__REGISTER and self.responseCodes == '401':
            ...
            self.server_addressS = socket.getaddrinfo(
                self.__proxyServer, self.pcmuPort)[0][-1]
            self.__sipACK(self.__userB, self.__userA, self.__auth)
    ...
    def __recive(self):
        try:
            data = self.__sock.recvfrom(180)
            a = uPySip.DTMF.DTMF().getKey(data[0])
            if a[1] > 500000:
